...
namespace Microsoft.ReverseProxy.ServiceFabric
{
    ...
    {
        public static string HealthReportSourceId = "Proxy";
            ...
            foreach (var application in applications)
            {
                ...
                foreach (var service in services)
                {
                    try
                    {
                        ...
                        var endpointNames = LabelsParser.GetEndpointNames(serviceExtensionLabels);

                        foreach (var endpointName in endpointNames)
                        {
                            // If this service wants to use proxy
                            if (serviceExtensionLabels.GetValueOrDefault($"{ConfigurationValues.EndpointsLabelPrefix}{ConfigurationValues.KeyDelimiter}{endpointName}{ConfigurationValues.KeyDelimiter}Enable", null) != "true")
                            {
                                // Skip this service
                                continue;
                            }

                            var cluster = LabelsParser.BuildCluster(service.ServiceName, endpointName, serviceExtensionLabels);
                            await DiscoverDestinationsAsync(cluster, options, service, endpointName, serviceExtensionLabels, cancellation);
                            ...
                            {
                                throw new ConfigException($"Duplicated cluster id '{cluster.Id}'. Skipping repeated definition, service '{service.ServiceName}', enpoint '{endpointName}'");
                            }

                            var routes = LabelsParser.BuildRoutes(service.ServiceName, endpointName, serviceExtensionLabels);
        ...
        private async Task DiscoverDestinationsAsync(
            ...
            string endpointName,
            IDictionary<string, string> serviceExtensionLabels,
            CancellationToken cancellation)
        {
            ...
            var endpointLabelPrefix = $"{ConfigurationValues.EndpointsLabelPrefix}{ConfigurationValues.KeyDelimiter}{endpointName}";
            var listenerName = serviceExtensionLabels.GetValueOrDefault($"{endpointLabelPrefix}{ConfigurationValues.KeyDelimiter}Backend{ConfigurationValues.KeyDelimiter}ServiceFabric{ConfigurationValues.KeyDelimiter}ListenerName", endpointName);
            var healthListenerName = serviceExtensionLabels.GetValueOrDefault($"{endpointLabelPrefix}.Backend{ConfigurationValues.KeyDelimiter}Healthcheck{ConfigurationValues.KeyDelimiter}ServiceFabric{ConfigurationValues.KeyDelimiter}ListenerName", string.Empty);
            var statefulReplicaSelectionMode = ParseStatefulReplicaSelectionMode(endpointName, serviceExtensionLabels);
            foreach (var partition in partitions)
            {
                ...
                foreach (var replica in replicas)
                {
                    ...
                    try
                    {
                        var destination = BuildDestination(replica, endpointName, healthListenerName);

                        ReportReplicaHealth(options, service, partition, replica, HealthState.Ok, $"Successfully built the endpoint from listener '{endpointName}'.");
        ...
        private StatefulReplicaSelectionMode ParseStatefulReplicaSelectionMode(string endpointName, IDictionary<string, string> serviceExtensionLabels)
        {
            var endpointLabelPrefix = $"{ConfigurationValues.EndpointsLabelPrefix}{ConfigurationValues.KeyDelimiter}{endpointName}";
            // Parse the value for StatefulReplicaSelectionMode: case insensitive, and trim the white space.
            var statefulReplicaSelectionMode = serviceExtensionLabels.GetValueOrDefault($"{endpointLabelPrefix}{ConfigurationValues.KeyDelimiter}ServiceFabric{ConfigurationValues.KeyDelimiter}StatefulReplicaSelectionMode", StatefulReplicaSelectionLabel.All).Trim();
